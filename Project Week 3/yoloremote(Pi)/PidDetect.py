...
def run_camera_thread():
    ...
    global latest_jpeg, robot_state
    ...
    while True:
        ...
        # 3. PID Logic OR YOLO Override
        if robot_state == "stop":
            # Laptop saw an object, override motors!
            set_motor_speed(0, 0)
            cv2.putText(frame_bgr, "YOLO OVERRIDE: STOPPED", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)
        else:
...
def generate_frames():
    ...
    while True:
        ...
        time.sleep(0.03) # Network speed limit
...
def handle_command(action):
    global robot_state
    if action in ["stop", "running"]:
        robot_state = action
        return f"Robot state changed to: {action}"
...
if __name__ == '__main__':
    try:
        # Start the Background Camera & Driving Thread
