...
PAUSE_DURATION = 4.0      # Total time to remain stopped and scan
FORWARD_DURATION = 0.35    # How long to drive straight over the symbol after pausing
COAST_DURATION = 0.25      # DELAY: How long to keep driving after detecting a symbol before stopping

# --- NEW: REVERSE PARAMETERS ---
REVERSE_DURATION = 0.4     # How long to reverse to fix network latency overshoot
REVERSE_SPEED = 40         # Speed for reversing
# -------------------------------
...
# ==========================================
def process_mask(mask, frame):
    results = []
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    for cnt in contours:
        if cv2.contourArea(cnt) > 280:
            ...
            # --- ONLY DISPLAY DIRECTION (Fixed Overlap) ---
            label = f"{direction}"
            results.append(label)
            
            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
            cv2.putText(frame, label, (x, y + h + 35), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 3)
                
    return results

def detect_arrows(frame, hsv):
    # We put all valid color ranges into a single list
    color_ranges = [
        [(35, 40, 40), (85, 255, 255)],     # Green
        [(100, 100, 50), (130, 255, 255)],  # Blue
        [(9, 80, 80), (25, 255, 255)],      # Orange
        [(0, 70, 70), (10, 255, 255)],      # Red 1
        [(160, 70, 70), (180, 255, 255)]    # Red 2
    ]

    # Create a blank black mask
    master_mask = np.zeros(hsv.shape[:2], dtype=np.uint8)

    # Combine all colors into the single master mask
    for lower, upper in color_ranges:
        mask = cv2.inRange(hsv, np.array(lower), np.array(upper))
        master_mask = cv2.bitwise_or(master_mask, mask)

    # Process the single combined mask
    return process_mask(master_mask, frame)
...
def pause_robot(symbol_class):
    """Endpoint triggered by Javascript to activate OpenCV analysis and stop the car."""
    global cv_arrow_active, robot_state, coast_start_time
    
    # 1. Force the robot to enter its stopping maneuver
    if robot_state == "FOLLOWING":
        robot_state = "COASTING"
        coast_start_time = time.time()
        print(f"\n[!] ML Triggered Stop Maneuver for: {symbol_class}")

    # 2. ALWAYS turn on the OpenCV math when a symbol is detected
...
def generate_frames():
    ...
    while True:
        ...
        if robot_state == "COASTING":
            ...
            # --- THE FIX: ONLY REVERSE IF IT IS AN ARROW ---
            if cv_arrow_active and elapsed_time < REVERSE_DURATION:
                set_motor_speed(-REVERSE_SPEED, -REVERSE_SPEED)
                cv2.putText(frame_bgr, "STATE: REVERSING", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)
                ...
                # Normal 10-second stop begins *after* reversing is done (or immediately for shapes)
                stop_motors() 
        ...
        # ---------------------------------------------------------
        # RUN OPENCV MATH IMMEDIATELY IF FLAG IS ACTIVE
        # ---------------------------------------------------------
        if cv_arrow_active:
