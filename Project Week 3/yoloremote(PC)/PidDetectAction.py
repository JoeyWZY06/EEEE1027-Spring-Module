...
is_maneuvering = False  # NEW: Prevents YOLO from spamming commands while moving
...
def execute_maneuver(action):
    ...
    if action == "stop_2s":
        ...
        # Spins in place. Adjust time.sleep if it under/over rotates
        turn_left(70, 2.0)
...
def run_camera_thread():
    """THREAD 1: Photographer & Line Follower Engine"""
    global latest_jpeg, robot_state, is_maneuvering
    ...
    while True:
        ...
        if ACTIVE_SHORTCUT:
            # --- UPDATED: Check for the combined color option ---
        ...
        # --- UPDATED: PID Logic OR YOLO Override ---
        if is_maneuvering:
            # Let the background thread handle the motors! Just update the HUD.
...
def handle_command(action):
    ...
    if action == "running":
        # Only reset to running if we aren't currently executing a maneuver
        if not is_maneuvering:
            robot_state = "running"
    elif not is_maneuvering: 
        # If laptop sends a maneuver and we aren't busy, trigger the thread!
        robot_state = action
